...
export async function getArticles(query: ArticlesParam = {}): Promise<{ list: Article[]; totalCount: number }> {
  const queryParam = new URLSearchParams(Object.entries(query)).toString();
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/articles?${queryParam}`);

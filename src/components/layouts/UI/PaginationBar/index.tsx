...
const PaginationBar = ({ onClick, order, totalPageNum }: Props) => {
  ...
  const startNum = Math.floor((activeNum - 1) / PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE) * PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE + 1;

  const pages = Array.from(
    {
      length: Math.min(PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE, Math.ceil(totalPageNum - startNum) + 1),
    },
    (_, i) => startNum + i,
  );
  ...
  useEffect(() => {
    const divActiveNum = Math.floor((activeNum - 1) / PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE);
    const divTotalPageNum = Math.floor((totalPageNum - 1) / PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE);
    if (activeNum > PAGE_RELATION_NUMBER.MAX_PAGINATION_SIZE) setDisabledArrowLeft(false);
  ...
  return (
    <div className={styles['pagination']}>
      <button className={styles['page-arrow-btn']} disabled={disabledArrowLeft} onClick={() => prevClick()}>
        <img src={arrowLeft} alt='좌측 화살표' />
      </button>
      {pages.map((value) => {
        const active = activeNum === value ? 'active-page-num' : '';
        return (
          <button key={value} className={`${styles['page-num-btn']} ${styles[active]}`} onClick={() => pageSelect(value)}>
            {value}
          </button>
        );
      })}
      <button className={styles['page-arrow-btn']} disabled={disabledArrowRight} onClick={() => nextClick()}>

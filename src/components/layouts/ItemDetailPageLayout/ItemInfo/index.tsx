...
const ItemInfo = () => {
  ...
  const errorImg = () => {
    if (imgRef.current !== null) imgRef.current.src = defaultItemImg;
  };
  ...
  const { images, name, price, description, tags, ownerNickname, createdAt, favoriteCount, ownerImages } = item;

  return (
    <section className='flex flex-col gap-6 border-b border-gray-200 mt-36 mx-auto pb-10 w-4/5 md:flex-row sm:mt-20'>
      <img src={images[0] ?? defaultItemImg} alt='상품 이미지' onError={errorImg} ref={imgRef} className='rounded-xl object-cover w-5/5 aspect-square max-w-md md:w-2/5' />
      ...
      <div className='flex flex-col justify-between relative flex-1'>
        ...
        <img src={dropdownImg} alt='드롭다운 이미지' className='absolute top-0 right-0 cursor-pointer hover:bg-gray-200' onClick={clickDropdownHandler} />
        {isDropdown && (
        ...
        <div>
          ...
          <div className='flex flex-wrap gap-2'>{tags?.map((value) => <span className='rounded-3xl py-1.5 px-4 bg-gray-100 text-gray-800 font-normal' key={value}>{`#${value}`}</span>)}</div>
        </div>

        <div className='flex items-center gap-4 mt-12 '>
          <img src={ownerImages?.[0] ?? emptyProfileImg} alt='상품 소유자 프로필 이미지' className='w-10 h-10' />
